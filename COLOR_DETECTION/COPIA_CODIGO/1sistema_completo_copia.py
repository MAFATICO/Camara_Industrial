...
CORES_DETECCAO = {
    ...
    "PRETO": (np.array([0, 0, 0]), np.array([150, 255, 50]), (60, 60, 60))  # array da cor preta
}
...
class VisionProApp(QMainWindow):
    ...
    def atualizar_painel_falhas(self):
        ...
        linhas = [f"- {motivo}: {contagem}" for motivo, contagem in
                  sorted(falhas_modelo.items(), key=lambda x: x[1], reverse=True)]
    ...
    def atualizar_loop(self):
        ...
        if self.aba_ativa == "TREINAR":
            ...
            if self.inspecao_ativa:
                ...
                cv2.putText(display_frame, f"STATUS: {status}", (w - 300, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, cor_status,
                            3)
                if motivo_fail:
    ...
    def exibir_frame(self, frame):
        ...
        pix = QPixmap.fromImage(qimg).scaled(new_w, new_h, Qt.AspectRatioMode.KeepAspectRatio,
