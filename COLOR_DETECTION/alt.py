...
DB_NAME = os.path.join(BASE_DIR, "../sistema_industrial.db")
DIR_DATASET = os.path.join(BASE_DIR, "dataset_ia")
...
class LoginScreen(QWidget):
    ...
    def init_ui(self):
        ...
        container.setStyleSheet("""
            ...
            QLineEdit { background-color: #0F1115; border: 1px solid #3E4451; color: white; padding: 10px; border-radius: 5px; margin-bottom: 10px; }
...
class VisionProApp(QMainWindow):
    def __init__(self):
        ...
        # CLAHE partilhado (usado em pré-processamento e referência)
        self.clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))

        # ── NOVO: intervalo de cor dinâmico (aprende da referência) ──────────
        # Se None, usa os intervalos fixos de CORES_DETECCAO como fallback
        self.intervalo_cor_dinamico = None

        self.roi_inspector   = ROIInspection(db_path=DB_NAME)
        self.historico_status = deque(maxlen=10)   # aumentado de 7 → 10
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # NOVO MÉTODO — Pré-processamento de iluminação (LAB + CLAHE)
    # ──────────────────────────────────────────────────────────────────────────
    def pre_processar_frame(self, frame):
        """
        Normaliza a iluminação do frame sem distorcer as cores.
        Converte para LAB, aplica CLAHE só no canal L (luminosidade)
        e volta para BGR. Deve ser chamado ANTES de qualquer deteção.
        """
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # NOVO MÉTODO — Calibração de cor a partir da referência
    # ──────────────────────────────────────────────────────────────────────────
    def calibrar_cor_referencia(self, frame_ref):
        """
        Aprende os intervalos HSV directamente dos pixels da peça de referência
        dentro das ROIs definidas. Chama após guardar a foto de referência.
        Devolve (low, high) ou None se não houver ROIs/pixels suficientes.
        """
        # Obtém máscara das ROIs (union de todos os polígonos)
        if not self.roi_inspector.poligonos:
            print("[Calibração] Sem ROIs definidas — a usar intervalos fixos.")
            return None

        h, w = frame_ref.shape[:2]
        mascara_roi = np.zeros((h, w), dtype=np.uint8)
        for poly in self.roi_inspector.poligonos:
            pts = np.array(poly, dtype=np.int32)
            if len(pts) > 2:
                cv2.fillPoly(mascara_roi, [pts], 255)

        # Normaliza iluminação antes de amostrar
        frame_norm = self.pre_processar_frame(frame_ref)
        hsv = cv2.cvtColor(frame_norm, cv2.COLOR_BGR2HSV)

        # Extrai pixels dentro das ROIs
        pixels = hsv[mascara_roi > 0]
        if len(pixels) < 50:
            print("[Calibração] Pixels insuficientes na ROI — a usar intervalos fixos.")
            return None

        # Calcula intervalo estatístico: média ± margem
        media  = np.mean(pixels, axis=0)
        margem = np.array([18, 45, 45], dtype=np.float32)  # tolerância H, S, V

        low  = np.clip(media - margem, [0,   0,   0],   [179, 255, 255]).astype(np.uint8)
        high = np.clip(media + margem, [0,   0,   0],   [179, 255, 255]).astype(np.uint8)

        print(f"[Calibração] Intervalos aprendidos → low={low}, high={high}")
        return low, high
    ...
    def _apply_style(self):
        self.setStyleSheet("""
            ...
            QPushButton#NavBtn { background: transparent; border: none; color: #848da0; text-align: left; padding: 12px 20px; font-weight: bold; font-size: 13px; }
            ...
            QPushButton#ModelBtn { background: #111; border: 1px solid #222; color: #888; padding: 10px; text-align: left; border-radius: 4px; margin-bottom: 2px; }
            QPushButton#ModelBtn:checked { border-color: #FF8C00; color: #FF8C00; background: #1A1A1A; }
            QPushButton#ControlBtn { background: #21252B; border: 1px solid #3E4451; color: white; padding: 10px 15px; border-radius: 6px; font-weight: bold; font-size: 11px; }
            QPushButton#ControlBtn:hover { border-color: #FF8C00; background: #2C313C; }
            QPushButton#DangerBtn { background: #2A1B1B; border: 1px solid #4A2626; color: #FF6666; padding: 10px 15px; border-radius: 6px; font-weight: bold; font-size: 11px; }
    ...
    def inicializar_bd(self):
        try:
            ...
            # Migração: adiciona colunas de cor se a tabela já existia sem elas
            for col, tipo in [("cor_low", "TEXT"), ("cor_high", "TEXT")]:
                try:
    ...
    def carregar_dados_peca(self, id_p):
        ...
        # ── NOVO: carrega intervalos de cor guardados ─────────────────────────
        cor_low_json  = p[11] if len(p) > 11 else None
        cor_high_json = p[12] if len(p) > 12 else None
        if cor_low_json and cor_high_json:
            self.intervalo_cor_dinamico = (
                np.array(json.loads(cor_low_json),  dtype=np.uint8),
                np.array(json.loads(cor_high_json), dtype=np.uint8),
            )
            print(f"[BD] Intervalos de cor carregados para '{self.peca_ativa_nome}'")
        else:
            self.intervalo_cor_dinamico = None
    ...
    def processar_referencia(self, img):
        """Extrai contorno de referência para matchShapes."""
        if img is None:
            return
        # ── NOVO: normaliza iluminação antes de processar a referência ────────
        img_norm = self.pre_processar_frame(img)
    ...
    def executar_inspecao(self, frame):
        ...
        # ── NOVO: normaliza iluminação ANTES de qualquer análise ─────────────
        frame_norm = self.pre_processar_frame(frame)

        resultado_roi = self.roi_inspector.processar_frame_completo(frame_norm)
        ...
        # ── NOVO: usa frame normalizado para deteção de cor ───────────────────
        hsv = cv2.cvtColor(frame_norm, cv2.COLOR_BGR2HSV)

        # ── NOVO: kernel morfológico para limpar ruído ─────────────────────────
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))

        objetos = []

        # ── NOVO: usa intervalo dinâmico se disponível, senão usa os fixos ────
        if self.intervalo_cor_dinamico is not None:
            low, high = self.intervalo_cor_dinamico
            ...
            for c in cnts:
                if cv2.contourArea(c) > 800:
                    objetos.append((cv2.contourArea(c), c, "COR_CALIBRADA"))
        else:
            # Fallback: intervalos fixos originais
        ...
        if self.modelo_aprendido is not None and \
           cv2.matchShapes(self.modelo_aprendido, c_prin, 1, 0.0) > 0.25:
            status_final = "FAIL"
    ...
    def estabilizar_status(self, status):
        """Voto por maioria em janela de 10 frames (era 7)."""
        if status == "AGUARDAR":
            self.historico_status.clear()
            return status
    ...
    def atualizar_loop(self):
        ...
        if self.aba_ativa == "TREINAR":
            ...
            if self.inspecao_ativa:
                ...
                # Indicador de modo de cor no ecrã
                modo_cor = "COR: CALIBRADA" if self.intervalo_cor_dinamico is not None else "COR: INTERVALOS FIXOS"
                cv2.putText(display_frame, modo_cor, (20, h - 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (0, 255, 100) if self.intervalo_cor_dinamico else (0, 150, 255), 1)
    ...
    def comando_salvar_s(self):
        ...
        if self.aba_ativa == "TREINAR":
            if self.frame_fixo_treino is None:
                ret, f = self.cap.read()
                if ret:
                    self.frame_fixo_treino = cv2.flip(f, 1)
                return
            ...
            if len(self.ponto_atual) > 2:
                ...
                resp = QMessageBox.question(
                    self, "Referência",
                    "Deseja salvar esta imagem como REFERÊNCIA?\n\n"
                    "(Sim = Salvar Ref + Calibrar Cor | Não = Descongelar | Cancelar = Nada)",
                ...
                if resp == QMessageBox.StandardButton.Yes:
                    ...
                    # ── NOVO: calibra cor e guarda na BD ─────────────────────
                    resultado_calibracao = self.calibrar_cor_referencia(self.frame_fixo_treino)
                    cor_low_json  = None
                    cor_high_json = None
                    if resultado_calibracao:
                        low, high = resultado_calibracao
                        self.intervalo_cor_dinamico = (low, high)
                        cor_low_json  = json.dumps(low.tolist())
                        cor_high_json = json.dumps(high.tolist())
                        print("[Salvar] Intervalos de cor calibrados e guardados na BD.")
                    else:
                        self.intervalo_cor_dinamico = None
                        print("[Salvar] Sem calibração de cor — a usar intervalos fixos.")

                    conn.execute(
                        "UPDATE pecas SET foto_ref=?, cor_low=?, cor_high=? WHERE id=?",
                        (buf.tobytes(), cor_low_json, cor_high_json, self.peca_ativa_id)
                    )
                    conn.commit()
                    conn.close()
    ...
    def salvar_progresso_bd(self):
        ...
        conn = sqlite3.connect(DB_NAME)
        conn.execute(
            "UPDATE pecas SET poligonos=?, aprovadas=?, rejeitadas=? WHERE id=?",
